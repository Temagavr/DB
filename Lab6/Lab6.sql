...
CREATE VIEW informatic_mark AS
	SELECT [mark].mark, [student].name FROM [mark]
...
SELECT * FROM informatic_mark;

DROP VIEW informatic_mark;
...
SELECT AVG([mark].mark) AS average_mark, [subject].name FROM [mark]
LEFT JOIN [student] ON [mark].id_student = [student].id_student
LEFT JOIN [lesson] ON [mark].id_lesson = [lesson].id_lesson
LEFT JOIN [subject] ON [lesson].id_subject = [subject].id_subject
...
CREATE VIEW VM_students_marks AS
	SELECT [student].name AS student_name, [group].name AS group_name, [mark].mark, [subject].name AS subject_name, [lesson].date FROM [student]
...
LEFT JOIN [lesson] ON [mark].id_lesson = [lesson].id_lesson
LEFT JOIN [subject] ON [lesson].id_subject = [subject].id_subject
WHERE [group].name = 'ПС' AND [subject].name = 'БД' AND [lesson].date < '2019-05-12'
